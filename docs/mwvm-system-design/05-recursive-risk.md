...
|---------------|-----------|-------------------|
| **Depth Limiter** | Configurable max nesting levels per bucket family (governance-set) | Each level after #1 requires +2% base $MORM lock |
| **Escalating Skin-in-the-Game** | Level 0: 1.5%; each level +2% (e.g. Level 1: 3.5%, Level 2: 5.5%, …) | All locked $MORM is first-loss buffer — absorbs first 25% drawdown |
...
- **Meta-Bucket Registration**: If user tries to wrap a bucket at max depth into new one → immediate revert
...
| **N (max)** | 9.5% (+2%) | Mandatory across entire tree | Same |
...
2. **Fee Amplification** — Performance fees tiered by depth (higher depth → higher fee %)
